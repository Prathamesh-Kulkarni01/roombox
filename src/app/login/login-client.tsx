/**
 * LOGIN PAGE CLIENT (Adaptive Auth)
 *
 * Branding-aware: When accessed from a PG subdomain (e.g. rahulpg.rentsutra.in/login),
 * the PG's logo and name are shown instead of the generic RentSutra branding.
 * On the main domain, renders the standard RentSutra login UI.
 *
 * Changes:
 ...
 * - Branding-aware: reads PG branding from BrandingContext.
 */
"use client";
...
export default function LoginPageClient() {
  ...
  const [challengeType, setChallengeType] =
    useState<ChallengeType>("PASSWORD_OR_OTP");
  ...
  useEffect(() => {
    console.log(
      "[LoginDebug] useEffect: currentUser.role =",
      currentUser?.role,
      "stage =",
      stage,
      "showSwitcher =",
    ...
    if (currentUser?.role) {
      ...
      const hasMultiple =
        (currentUser.activeTenancies?.length || 0) +
          (currentUser.activeStaffProfiles?.length || 0) >
        1;
      console.log(
        "[LoginDebug] hasMultiple:",
        hasMultiple,
        "lastActiveContext:",
        currentUser.lastActiveContext,
      );
      ...
      if (redirectParam) {
        console.log(
          "[LoginDebug] Redirecting to target parameter:",
          redirectParam,
        );
        ...
        console.log(
          "[LoginDebug] Redirecting to complete-profile (onboarding)...",
        );
        router.replace("/complete-profile");
      } else {
  ...
  const handleIdentityCheck = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(data.error || "Failed to check account state");
  ...
  const handlePasswordSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(
        "[LoginDebug] Sign-in successful for UID:",
        userCredential.user.uid,
      );
      // Let useEffect handle redirect/switching
    } catch (err: any) {
      console.error(
        "[LoginDebug] Password sign-in failed:",
        err.code,
        err.message,
        err,
      );
      ...
      if (err.code === "auth/user-not-found")
        msg = "Account not found. Use setup code if new.";
      toast({
        variant: "destructive",
        title: "Login Failed",
        description: msg,
      ...
      <div className="flex items-center justify-center min-h-[85vh] p-4 overscroll-none">
        <Card className="w-full max-w-sm border-primary/20 shadow-2xl">
          <CardHeader className="text-center space-y-1">
            ...
            <CardDescription>
              Enter your email to manage properties
            </CardDescription>
          </CardHeader>
    ...
    <div className="flex items-center justify-center min-h-[85vh] bg-background p-4 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary/10 via-background to-background overscroll-none">
      <Card className="w-full max-w-[400px] border-primary/10 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.1)] backdrop-blur-sm bg-card/95">
        <CardHeader className="text-center pb-8 pt-8">
          ...
          {branding?.logoUrl ? (
            <div className="mx-auto mb-5 relative w-16 h-16 rounded-2xl overflow-hidden shadow-lg border border-border/20">
              <Image
          ...
          <CardTitle className="text-3xl font-extrabold tracking-tight">
            {branding?.siteTitle ?? "RentSutra"}
          </CardTitle>
          <CardDescription className="text-base">
            {stage === "IDENTITY"
              ? branding
                ? `Welcome to ${branding.siteTitle} — sign in to continue`
                : "Experience your digital PG life"
              : stage === "SWITCH_CONTEXT"
                ? "Pick a profile to continue"
                : "Welcome back, authenticate to continue"}
          </CardDescription>

          {/* Powered by badge — only shown on branded subdomain */}
          {branding && (
            <p className="text-[10px] text-muted-foreground/50 font-medium mt-1">
              Powered by{" "}
              <Link
                href="https://rentsutra.in"
                target="_blank"
            ...
            <form onSubmit={handleIdentityCheck} className="grid gap-6">
              <div className="grid gap-3">
                ...
                <div className="relative group">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground font-medium border-r pr-3">
                    +91
                  </span>
                  <Input
                    id="phone"
                    ...
                    required
                    className="pl-14 h-12 text-lg font-medium tracking-wide focus-visible:ring-primary/20"
                    value={phone}
                    onChange={(e) =>
                ...
                <p className="text-[11px] text-muted-foreground text-center">
                  Enter the number shared with your property owner.
                </p>
              </div>

              <Button
                type="submit"
                className="w-full h-12 text-md font-bold group shadow-lg shadow-primary/10"
              ...
              <div className="flex flex-col gap-3 mt-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button
                    type="button"
                    variant="outline"
                    className="h-10 text-[11px] font-bold hover:bg-primary/5 hover:text-primary transition-colors px-1"
            ...
            <div className="grid gap-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-8 text-xs font-bold text-muted-foreground hover:bg-primary/5 hover:text-primary"
                  onClick={() => {
                    setStage("IDENTITY");
                ...
                <form onSubmit={handleVerifyCode} className="grid gap-6">
                  <div className="grid gap-3">
                    ...
                    <Input
                      ...
                      required
                      maxLength={6}
                      className="h-12 text-center text-2xl font-mono tracking-[0.5em] focus:tracking-[0.8em] transition-all"
                      value={otp}
                      onChange={(e) =>
                    ...
                    <p className="text-[11px] text-muted-foreground italic text-center">
                      This code was shared by your manager for setup.
                    </p>
                  </div>
                  <Button
                    type="submit"
                    ...
                    >
                      <div className="grid gap-3">
                        ...
                        <Input
                          ...
                          required
                          className="h-12 text-base md:text-sm focus-visible:ring-primary/20"
                          value={password}
                        ...
                        <button
                          type="button"
                          className="text-xs font-bold text-primary hover:underline text-left w-fit"
                      ...
                      <Button
                        type="submit"
                        className="w-full h-12 text-md font-bold shadow-lg shadow-primary/10"
                        ...
                        <div className="grid gap-4">
                          <div className="bg-primary/5 p-4 rounded-xl border border-primary/10">
                            <p className="text-xs text-center text-muted-foreground leading-relaxed">
                              Security Check: We will send a 6-digit code via
                              SMS to verify your identity.
                            </p>
                          </div>
                          <Button
                        ...
                        >
                          <div className="grid gap-3">
                            ...
                            <Input
                              ...
                              required
                              maxLength={6}
                              className="h-12 text-center text-2xl font-mono tracking-[0.5em] transition-all"
                              value={otp}
                              onChange={(e) =>
                                setOtp(e.target.value.replace(/\D/g, ""))
                              }
