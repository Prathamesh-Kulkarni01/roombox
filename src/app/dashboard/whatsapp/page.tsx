    ...
    events: [
      {
        ...
        title: "Rent Overdue",
        desc: "Sent when rent is overdue.",
        icon: AlertCircle,
        tenant: true,
        owner: false,
      },
      {
        id: "rent-reminder-upcoming",
        title: "Rent Reminder",
        desc: "Sent a few days before the due date.",
        icon: Bell,
        tenant: true,
        owner: false,
      },
      {
        id: "payment-confirmation-tenant",
        title: "Payment Confirmation (Tenant)",
        desc: "Digital receipt sent to tenant on payment.",
        icon: CheckCircle,
        tenant: true,
        owner: false,
      },
      {
        id: "payment-confirmation-owner",
        title: "Payment Received (Owner)",
        desc: "Instant alert to you when rent is paid online.",
        icon: IndianRupee,
        tenant: false,
        owner: true,
      },
    ...
    events: [
      {
        id: "new-complaint",
        title: "New Complaint",
        desc: "Alerts you when a tenant raises an issue.",
        icon: MessageCircle,
        tenant: false,
        owner: true,
      },
      {
        ...
        desc: "Informs tenant when their issue status changes.",
        icon: CheckCircle,
        tenant: true,
        owner: false,
      },
      {
        id: "announcement",
        title: "Notice Board",
        desc: "Broadcasts general announcements to tenants.",
        icon: MessageCircle,
        tenant: true,
        owner: false,
      },
    ...
    events: [
      {
        ...
        desc: "Welcomes new tenant and informs owner.",
        icon: UserPlus,
        tenant: true,
        owner: true,
      },
      {
        ...
        desc: "Informs tenant if KYC is approved or rejected.",
        icon: FileText,
        tenant: true,
        owner: false,
      },
      {
        id: "guest-exit",
        title: "Guest Exit Initiated",
        desc: "Notifies you when a guest starts their notice period.",
        icon: LogOut,
        tenant: false,
        owner: true,
      },
...
export default function WhatsAppPage() {
  ...
  const [notificationSettings, setNotificationSettings] = useState<
    Record<string, { tenant: boolean; owner: boolean }>
  >({});
    ...
    try {
      ...
      if (data.success) {
        toast({
          title: "Verified!",
          description:
            "Your WhatsApp number has been verified. You can now use Magic Login.",
        });
        setIsOtpSent(false);
        setOtp("");
      } else {
  ...
  const handleRecharge = async (amount: number) => {
    ...
    try {
      ...
      if (!success || !order)
        throw new Error(error || "Failed to create recharge order.");
      ...
      const options = {
        ...
        handler: (response: any) => {
          toast({
            ...
            description: "Your new balance will reflect shortly.",
          });
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: err.message || "Could not initiate recharge.",
      });
    } finally {
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center text-center p-8 bg-muted/50 rounded-lg border">
            ...
            <p className="mt-2 text-muted-foreground max-w-sm">
              WhatsApp Automation is a premium feature. Please recharge your
              wallet to automate your communications.
            </p>
            <Button
              className="mt-4"
              ...
              <div className="flex-1 grid grid-cols-2 gap-4 text-center w-full">
                <div className="p-3 border rounded-lg bg-background">
                  ...
                  <p className="font-bold text-lg">
                    ~{Math.floor(availableCredits / 1.5)}
                  </p>
                </div>
                <div className="p-3 border rounded-lg bg-background">
                  <p className="text-xs text-muted-foreground">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  {!isOtpSent ? (
                    <Button
                      ...
                      disabled={
                        isVerifying || !phoneNumber || phoneNumber.length < 10
                      }
                    >
                      {isVerifying ? (
                ...
                {isOtpSent && (
                  <div className="space-y-2 pt-2 animate-in fade-in slide-in-from-top-1">
                    ...
                    <Button
                      size="sm"
                      className="w-full h-8 bg-green-600 hover:bg-green-700 font-bold"
                            ...
                            <TableRow key={event.id}>
                              ...
                              <TableCell>
                                <Switch
                                  ...
                                  checked={
                                    event.tenant &&
                                    !!notificationSettings[event.id]?.tenant
                                  }
                                  onCheckedChange={() =>
                                    handleToggle(event.id, "tenant")
                                  }
                                />
                              </TableCell>
                              <TableCell>
                                <Switch
                                  ...
                                  checked={
                                    event.owner &&
                                    !!notificationSettings[event.id]?.owner
                                  }
                                  onCheckedChange={() =>
                                    handleToggle(event.id, "owner")
                                  }
                                />
                              </TableCell>
                            </TableRow>
                          ...
                          <TableRow key={log.id}>
                            ...
                            <TableCell>
                              ...
                              {log.direction === "inbound" && (
                                <Badge className="ml-1 bg-blue-100 text-blue-700 text-[10px]">
                                  IN
                                </Badge>
                              )}
                            </TableCell>
