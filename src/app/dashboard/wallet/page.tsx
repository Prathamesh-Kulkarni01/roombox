...
import { calculateLowBalanceStage, cn, getCurrentPlan } from "@/lib/utils";
  ...
  const handleToggleFeature = (
    ...
    startTransition(async () => {
      const resultAction = await dispatch(
        togglePremiumFeature({ feature, enabled }),
      );
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto p-4 md:p-6 lg:p-8 animate-in fade-in duration-700">
      ...
      <RechargeDialog
        ...
        maxBedCount={
          billingDetails?.details.totalBeds ||
          billingDetails?.details.billableTenantCount ||
          0
        }
        baseFee={baseFee}
      />
            ...
            <CardContent className="space-y-6 p-6 relative z-10">
              <div className="text-center py-2">
                <div className="flex items-center justify-center gap-1">
                  <span className="text-2xl font-black text-muted-foreground/50 self-start mt-1">
                    ₹
                  </span>
                  <p
                    className={cn(
                      ...
                      walletBalance <= 0
                        ? "text-red-600"
                        : walletBalance <=
                            PRICING_CONFIG.lowBalance.riskThreshold
                          ? "text-amber-600"
                          : "text-emerald-600",
                    )}
                  >
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] font-medium text-muted-foreground leading-relaxed">
                    Charges are deducted daily at midnight. No manual bills to
                    pay.
                  </p>
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] font-medium text-muted-foreground leading-relaxed">
                    You are only charged for active tenants. Empty beds are
                    free.
                  </p>
                </div>
                <div className="pt-2">
                  <Button
                    variant="link"
                    className="p-0 h-auto text-[10px] font-black uppercase text-primary"
                  >
                    View Detailed FAQ
                  </Button>
                </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section id="features">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(PRICING_CONFIG.premiumFeatures).map(
                ([key, feature]: [string, any]) => {
                  const isEnabled =
                    currentUser.subscription?.premiumFeatures?.[
                      key as keyof PremiumFeatures
                    ]?.enabled || false;
                  return (
                    <Card
                      key={key}
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground font-medium mt-1 leading-relaxed">
                            {featureDescriptions[key] ||
                              "Enhance your property management with advanced tools."}
                          </p>
                        </div>
                        <div className="pt-2 border-t border-border/50 flex justify-between items-center">
                          ...
                          <p className="text-xs font-black text-primary">
                            {feature.billingType === "monthly"
                              ? `₹${feature.monthlyCharge} / Month`
                              : `₹${feature.perTenantCharge} / Bed / Month`}
                          </p>
                        </div>
                      </CardContent>
                    </Card>
                ...
                {billingDetails && (
                  <div className="flex items-center gap-3 bg-card px-4 py-2 rounded-xl border shadow-sm">
                    ...
                    <p className="text-xl font-black text-primary">
                      ₹
                      {(
                        billingDetails.currentCycle.totalAmount || 0
            ...
            <CardContent className="p-6">
              {isLoadingBill ? (
                ...
                <div className="space-y-4">
                    ...
                    <div className="p-4 rounded-2xl bg-card border border-border/50 shadow-sm">
                      ...
                      <div className="flex justify-between items-end">
                        ...
                        <p className="text-lg font-black italic tracking-tighter">
                          ₹{(baseFee || 0).toLocaleString("en-IN")}
                        </p>
                      </div>
                    </div>
                    <div className="p-4 rounded-2xl bg-card border border-border/50 shadow-sm">
                      ...
                      <div className="flex justify-between items-end">
                        ...
                        <p className="text-lg font-black italic tracking-tighter">
                          ₹
                          {(
                            billingDetails.currentCycle.tenantCharge || 0
                          ).toLocaleString("en-IN")}
                        </p>
                      </div>
                    </div>
                  ...
                  {Object.keys(
                    billingDetails.currentCycle.premiumFeaturesDetails || {},
                  ).length > 0 && (
                        ...
                        {Object.entries(
                          ...
                          >
                            ...
                            <span className="font-black text-sm italic">
                              ₹{(feature.charge || 0).toLocaleString("en-IN")}
                            </span>
                          </div>
                        ))}
                      </div>
                  ...
                  {billingDetails.currentCycle.discountAmount &&
                    billingDetails.currentCycle.discountAmount > 0 && (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent bg-muted/20 border-b border-border/40">
                    <TableHead className="px-6 h-12 font-black uppercase text-[0.65rem] tracking-widest text-muted-foreground">
                      Date
                    </TableHead>
                    <TableHead className="px-6 h-12 font-black uppercase text-[0.65rem] tracking-widest text-muted-foreground">
                      Amount
                    ...
                    <TableHead className="text-right px-6 h-12 font-black uppercase text-[0.65rem] tracking-widest text-muted-foreground">
                      Status
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {walletTxns.length > 0 ? (
                    walletTxns.map((txn) => (
                      <TableRow
                        key={txn.id}
                        className="hover:bg-muted/20 transition-colors border-b border-border/40 last:border-0"
                      >
                        <TableCell className="px-6 py-5">
                          <p className="font-bold text-sm">
                            {format(
                              typeof txn.createdAt === "string"
                                ? parseISO(txn.createdAt)
                                : (txn.createdAt as any),
                              "do MMM, yyyy",
                            )}
                          </p>
                        ...
                        <TableCell className="px-6 py-5">
                          <Badge
                            variant="outline"
                            className="rounded-full px-3 py-0.5 font-bold text-[0.65rem] uppercase tracking-widest"
                          >
                    ...
                    <TableRow>
                      <TableCell
                        colSpan={4}
                        className="text-center h-48 text-muted-foreground font-medium italic"
                      >
