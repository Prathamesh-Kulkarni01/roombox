                ...
                <div className="hidden md:flex flex-col items-end">
                    <div className="group relative inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-emerald-500/5 text-emerald-700 border border-emerald-200/50 shadow-sm transition-all hover:bg-emerald-500/10 hover:border-emerald-500/30">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[0.6rem] font-black uppercase tracking-widest text-emerald-600/70">Secure Node</span>
                            <span className="text-sm font-bold tracking-tight">Verified Merchant</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="px-4 md:px-0">
                <Card className="bg-gradient-to-br from-indigo-600 via-blue-600 to-blue-700 border-none shadow-2xl rounded-[2.5rem] overflow-hidden group relative">
                    ...
                    <CardContent className="p-6 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 text-white relative z-10">
                        ...
                        <ArrowRight className="hidden md:block w-8 h-8 text-white/20 group-hover:text-white/40 group-hover:translate-x-2 transition-all" />
                    </CardContent>
                </Card>
            </div>

            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
                <TabsList className="grid grid-cols-2 w-full lg:w-[450px] h-16 p-1.5 bg-muted/30 backdrop-blur-sm rounded-[1.25rem] border border-border/40">
                    <TabsTrigger value="offline" className="rounded-2xl data-[state=active]:bg-white data-[state=active]:shadow-lg data-[state=active]:text-primary transition-all duration-300 text-sm font-black uppercase tracking-widest gap-2">
                        <CreditCard className="w-4 h-4" /> Offline Setup
                    </TabsTrigger>
                    <TabsTrigger value="online" className="rounded-2xl data-[state=active]:bg-white data-[state=active]:shadow-lg data-[state=active]:text-primary transition-all duration-300 text-sm font-black uppercase tracking-widest gap-2">
                        <Zap className="w-4 h-4" /> Online Setup
                    </TabsTrigger>
                </TabsList>

                <TabsContent value="offline" className="space-y-8 animate-in fade-in slide-in-from-left-4 duration-500 outline-none">
                    <div className="flex flex-col gap-2 mb-2 p-1">
                        <h2 className="text-2xl font-black tracking-tight">Direct UPI & Cash</h2>
                        <p className="text-muted-foreground font-medium">Configure QR codes and UPI IDs for zero-fee offline collections.</p>
                    </div>
                    <PaymentSettings onSwitchToOnline={() => setActiveTab('online')} />
                </TabsContent>

                <TabsContent value="online" className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 outline-none">
                    {process.env.NODE_ENV === 'production' ? (
                        <Card className="border-blue-500/20 bg-blue-500/[0.02] shadow-2xl rounded-[2.5rem] overflow-hidden">
                            <CardContent className="p-20 text-center space-y-6">
                                ...
                                <h3 className="text-3xl font-black text-blue-900">Online gateway payments coming soon.</h3>
                                <p className="text-blue-700/60 font-medium max-w-md mx-auto">
                                    We're working on a seamless automated rent collection experience. Stay tuned!
                                </p>
                            </CardContent>
                        </Card>
                    ) : (
                                        ...
                                        <FormControl>
                                            <RadioGroup onValueChange={field.onChange} value={field.value} className="grid grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <label htmlFor="bank_account" className="flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-muted bg-popover p-6 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/[0.03] [&:has([data-state=checked])]:border-primary transition-all cursor-pointer shadow-sm group">
