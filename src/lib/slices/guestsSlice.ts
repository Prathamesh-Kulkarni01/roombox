...
// Async Thunks
export const addGuest = createAsyncThunk<{ newGuest: Guest; updatedPg: PG, existingUser?: User, whatsAppStatus: 'sent' | 'failed' | 'skipped' }, NewGuestData, { state: RootState }>(
    'guests/addGuest',
...
export const reconcileGuestPayment = createAsyncThunk<{ guest: Guest; whatsAppStatus: 'sent' | 'failed' | 'skipped' }, {
    guest: Guest;
    ...
    async ({ guest, paymentAmount, paymentMethod }, { getState, dispatch, rejectWithValue }) => {
        ...
        const { guest: reconciledGuest } = runReconciliationLogic(guestWithPayment, now);

        if (user.currentPlan?.hasCloudSync && isFirebaseConfigured()) {
            const selectedDb = selectOwnerDataDb(user.currentUser);
            const guestDocRef = doc(selectedDb, 'users_data', ownerId, 'guests', reconciledGuest.id);
            await setDoc(guestDocRef, reconciledGuest);
        }
        ...
        if (guest.userId) {
            ...
            tenantNotificationResult.whatsAppStatus = result.whatsAppStatus;
        }

        return { guest: reconciledGuest, whatsAppStatus: tenantNotificationResult.whatsAppStatus };
    }
);
