...
export default function GuestPopoverContent({
  guest,
    ...
    <PopoverContent className="w-64 p-0">
      <div className="p-4 space-y-2">
        <div className="flex items-center gap-3">
          <Avatar>
            <AvatarImage
              src={`https://placehold.co/40x40.png?text=${guest.name.charAt(0)}`}
            />
        ...
        <div className="text-sm space-y-1">
          <div className="flex justify-between">
            ...
            <Badge
              variant="outline"
              className={cn("capitalize", rentStatusColors[guest.rentStatus])}
            >
              {guest.rentStatus}
            </Badge>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">
              {(() => {
                try {
                  const d = new Date(guest.dueDate);
                  return isNaN(d.getTime()) ? "N/A" : format(d, "do MMM");
                } catch {
                  return "N/A";
                }
              })()}
          ...
          {guest.exitDate && !guest.isVacated && (
            <div className="flex justify-between text-blue-600">
              ...
              <span className="font-medium">
                {differenceInDays(parseISO(guest.exitDate), new Date())} days
              </span>
            </div>
          )}
        </div>
      ...
      <div className="flex flex-col gap-1 p-2 bg-muted/50">
        {(guest.rentStatus === "unpaid" ||
          ...
          !guest.exitDate && (
            <Button
              ...
              onClick={() => handleOpenPaymentDialog(guest)}
            >
        ...
        canAccess(featurePermissions, currentUser?.role, "guests", "edit") ? (
          <Button
            ...
            size="sm"
            className="justify-start"
            onClick={() => setGuestToInitiateExit(guest)}
          >
        ...
        {guest.exitDate &&
          canAccess(
            ...
            <Button
              ...
              onClick={() => setGuestToExitImmediately(guest)}
            >
