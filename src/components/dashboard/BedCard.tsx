...
interface BedCardProps extends UseDashboardReturn {
  room: Room
  floor: Floor
  pg: PG
          ...
          return (
            <Popover key={bed.id}>
              <PopoverTrigger asChild>
                <button className={cn(`group relative border-2 rounded-lg aspect-square flex flex-col items-center justify-center p-2 transition-colors text-left w-full hover:brightness-105`, bedStatusClasses[status])}>
                  ...
                  <div className="w-full text-center mt-auto">
                    ...
                    {status === 'occupied' && (
                      <div className="text-xs text-green-600 font-bold flex items-center justify-center gap-1.5 py-1">
                        <CheckCircle className="w-3.5 h-3.5 text-green-500" /> Rent Paid
                      </div>
                    )}
              ...
              <GuestPopoverContent
                guest={guest}
                {...props}
                handleOpenAddGuestDialog={props.handleOpenAddGuestDialog}
                handleOpenEditGuestDialog={props.handleOpenEditGuestDialog}
                handleOpenPaymentDialog={props.handleOpenPaymentDialog}
                handleOpenReminderDialog={props.handleOpenReminderDialog}
                setGuestToInitiateExit={props.setGuestToInitiateExit}
                setGuestToExitImmediately={props.setGuestToExitImmediately}
              />
            </Popover>
          );
        })}
