...
const rentStatusConfig = {
  paid: {
    label: "Rent Paid ✓",
    color: "bg-green-500/15 text-green-700 border-green-300",
  },
  unpaid: {
    label: "Rent Due ⚠️",
    color: "bg-red-500/15 text-red-700 border-red-300",
  },
  partial: {
    label: "Partially Paid",
    color: "bg-orange-500/15 text-orange-700 border-orange-300",
  },
};
...
export default function BedActionSheet({
  ...
  const status = rentStatusConfig[guest.rentStatus] ?? {
    label: guest.rentStatus,
    color: "bg-muted text-muted-foreground border-border",
  };
  const canEditGuests = canAccess(
    featurePermissions,
    currentUser?.role,
    "guests",
    "edit",
  );
  const canDeleteGuests = canAccess(
    featurePermissions,
    currentUser?.role,
    "guests",
    "delete",
  ...
  const isUnpaid =
    guest.rentStatus === "unpaid" || guest.rentStatus === "partial";
  const daysUntilExit = guest.exitDate
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      ...
      >
        ...
        <div
          className={cn(
            "mx-4 mt-2 mb-4 rounded-2xl p-4 flex items-center gap-4",
        ...
        >
          ...
          <div className="shrink-0 text-right">
            <Badge
              variant="outline"
              className={cn("text-xs font-semibold border", status.color)}
            >
              {status.label}
            </Badge>
            {totalDue > 0 && (
        ...
        <div className="px-4 pb-6 grid grid-cols-2 gap-3">
          ...
          {currentPlan?.hasAiRentReminders &&
            ...
            !guest.exitDate && (
              <button
                onClick={() =>
                  handleAction(() => handleOpenReminderDialog(guest))
                }
          ...
          {canEditGuests && (
            <button
              onClick={() =>
                handleAction(() => handleOpenTransferDialog(guest))
              }
              className="flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-muted hover:bg-muted/70 font-semibold text-sm active:scale-95 transition-transform"
            >
          ...
          {canEditGuests && (
            <button
              onClick={() =>
                handleAction(() => handleOpenEditGuestDialog(guest))
              }
              className="flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-muted hover:bg-muted/70 font-semibold text-sm active:scale-95 transition-transform"
            >
          ...
          {guest.exitDate && canDeleteGuests && (
            <button
              onClick={() =>
                handleAction(() => setGuestToExitImmediately(guest))
              }
