...
import { MoreHorizontal, Home, BookUser, MessageSquareWarning, CreditCard, ChevronRight, Wallet, Building, UserCircle, LogOut, LayoutGrid, BedDouble, Plus, LayoutDashboard } from 'lucide-react';
...
// ... (rest of getInsight stays same)
    ...
    if (href === '/dashboard/complaints') {
      const openCount = (filteredComplaints || []).filter(c => c && c.status === 'open').length;
      return openCount > 0 ? { label: String(openCount), variant: 'destructive' as const } : undefined;
    }
    if (href === '/dashboard/pg-management') {
      const totalRooms = (filteredPgs || []).reduce((acc, p) => acc + (p?.totalRooms || 0), 0);
      return totalRooms > 0 ? { label: String(totalRooms), variant: 'secondary' as const } : undefined;
    }
    if (href === '/dashboard/rent-passbook') {
      const totalDues = (filteredGuests || []).reduce((acc, g) => acc + (g?.balance || 0), 0);
      return totalDues > 0 ? { label: totalDues >= 1000 ? `${(totalDues/1000).toFixed(1)}k` : String(totalDues), variant: 'destructive' as const } : undefined;
    }
    if (href === '/dashboard/wallet') {
      const balance = currentUser?.wallet?.balance || 0;
      return { label: `₹${balance}`, variant: balance < 100 ? 'destructive' : 'default' as any };
    }
    return undefined;
  };

  const mainNavItems = [
    { href: '/dashboard', label: 'nav_dashboard_short', icon: LayoutDashboard, feature: 'core' },
    { href: '/dashboard/pg-management', label: 'manage_rooms_short', icon: BedDouble, feature: 'properties' },
    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 glass border-t border-border/50 z-50 pb-safe">
      <nav className="grid grid-cols-5 h-16 items-center px-1">
        ...
        <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
          ...
          <SheetContent side="bottom" className="h-auto max-h-[90dvh] flex flex-col rounded-t-[2.5rem] p-0 border-t-0 glass pb-safe">
            ...
            <div className="px-6 py-5 flex items-center gap-4">
              ...
              <div className="flex-1 overflow-hidden">
                <div className="flex items-center gap-2">
                  <h3 className="font-black text-xl tracking-tight truncate">{currentUser.name}</h3>
                  <Badge variant="outline" className="text-[9px] font-black uppercase tracking-tighter h-4 px-1.5 bg-primary/5 text-primary border-primary/20">
                    {currentPlan.name}
                  </Badge>
                </div>
