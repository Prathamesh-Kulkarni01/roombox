...
export async function up(db: admin.firestore.Firestore, isDryRun?: boolean): Promise<MigrationResult> {
    ...
    // Update 'complaints' collection group for schemaVersion
    const complaintsSnap = await db.collectionGroup('complaints').get();
    ...
    for (const doc of complaintsSnap.docs) {
        ...
        if (needsUpdate) {
            try {
                ...
                console.error(`Failed to update complaint ${doc.id}:`, err);
                result.errors++;
            }
        }
    }
