...
const cronJobs = [
  {
    ...
    expectedSignals: ['runMonthlyBillingCron', 'authorization', 'CRON_SECRET'],
  },
  {
    slug: 'reconcile-rent',
    description: 'Rent reconciliation and accounting sync',
    routePath: 'src/app/api/cron/reconcile-rent/route.ts',
    expectedSignals: ['reconcileAllGuests', 'authorization', 'CRON_SECRET'],
  },
  {
  ...
  },
  {
    ...
    expectedSignals: ['runMaintenanceCron', 'authorization', 'CRON_SECRET'],
  },
] as const;
...
export async function auditCronJobs(): Promise<CronJobAuditReport> {
  const jobs = await Promise.all(
    cronJobs.map(async (job) => {
      ...
      if (!/authorization|CRON_SECRET/.test(source)) {
